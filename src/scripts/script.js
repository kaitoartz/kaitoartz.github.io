...
class AudioManager {
    ...
    handleMouseOver(e) {
        ...
        const selector = 'a, button, input, textarea, .project-card, .filter-btn, .link-block';
        const target = e.target.closest(selector);

        // Optimization: Only play if we entered a NEW target
        if (target) {
            // Check if we are moving from a child to a parent that also matches the selector
            // We only want to trigger the sound if we enter a completely new element tree
