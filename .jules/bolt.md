...
## 2026-06-26 - State tracking for high-frequency events
**Learning:** Performing DOM reads or writes (like `classList.add` or `classList.remove`) unconditionally inside high-frequency event handlers like `scroll` causes redundant layout updates and DOM thrashing, even if the value hasn't changed.
**Action:** Use state variables to track the current applied state (e.g., `isScrollTopVisible`) and only touch the DOM when the incoming state differs from the cached state.
