...
## 2024-05-24 - DOM Manipulation Reflows in Initialization
**Learning:** Appending individual elements directly to a live DOM node (`this.world`) inside loops during intensive initialization sequences (like `HyperScrollIntro`) triggers expensive synchronous style recalculations and layout reflows per element.
**Action:** Always batch element creation into a `DocumentFragment` and append the fragment to the live DOM exactly once to reduce layout reflows to O(1).
